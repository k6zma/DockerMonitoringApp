...
func (e *ErrorHandlers) NotFoundHandler(w http.ResponseWriter, r *http.Request) {
	...
	e.logger.Warnf("REQUESTS: - %s - %s - %s - %d - %s", clientIP, r.Method, r.URL.Path, http.StatusNotFound, "0ms")
	http.Error(w, "404 page not found", http.StatusNotFound)
}

func (e *ErrorHandlers) MethodNotAllowedHandler(w http.ResponseWriter, r *http.Request) {
	clientIP := utils.GetClientIP(r)
	e.logger.Warnf("REQUESTS: - %s - %s - %s - %d - %s", clientIP, r.Method, r.URL.Path, http.StatusMethodNotAllowed, "0ms")
